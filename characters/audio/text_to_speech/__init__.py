...
def get_text_to_speech(tts: Optional[str] = None) -> TextToSpeech:
    if (
        ...
        tts = "EDGE_TTS"
    if tts == "ELEVEN_LABS":
        from characters.audio.text_to_speech.elevenlabs import ElevenLabs

        ElevenLabs.initialize()
        return ElevenLabs.get_instance()
    elif tts == "GOOGLE_TTS":
        from characters.audio.text_to_speech.google_cloud_tts import GoogleCloudTTS

        GoogleCloudTTS.initialize()
        return GoogleCloudTTS.get_instance()
    elif tts == "EDGE_TTS":
        from characters.audio.text_to_speech.edge_tts import EdgeTTS

        EdgeTTS.initialize()
        return EdgeTTS.get_instance()
    elif tts == "XTTS":
        from characters.audio.text_to_speech.xtts import XTTS
